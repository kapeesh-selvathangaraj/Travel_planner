...
const Home = () => {
  ...
  // Fetch the top-rated packages
  const getTopPackages = useCallback(async () => {
    if (topPackages.length > 0) return; // Prevent re-fetch if already fetched
    try {
      setLoading(true);
      ...
      console.error(error);
      alert("Error fetching top-rated packages");
    } finally {
      setLoading(false);
    }
  }, [topPackages]);
  ...
  const getLatestPackages = useCallback(async () => {
    if (latestPackages.length > 0) return; // Prevent re-fetch if already fetched
    try {
      setLoading(true);
      ...
      console.error(error);
      alert("Error fetching latest packages");
    } finally {
      setLoading(false);
    }
  }, [latestPackages]);

  // Fetch the offer packages
  const getOfferPackages = useCallback(async () => {
    if (offerPackages.length > 0) return; // Prevent re-fetch if already fetched
    try {
      setLoading(true);
      ...
      console.error(error);
      alert("Error fetching offer packages");
    } finally {
      setLoading(false);
    }
  }, [offerPackages]);
